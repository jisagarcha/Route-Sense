...
export default function OptimizePackagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (packageData) {
      ...
      // Add delivery locations - show all items even without coordinates
      packageData.items.forEach((item, index) => {
        // Use item coordinates if available, otherwise use default Kathmandu area coordinates
        // Spread items around Kathmandu area slightly for better visualization
        const baseLatOffset = (index % 5) * 0.01 - 0.02; // -0.02 to +0.02
        ...
        locations.push({
          ...
          description: item.deliveryAddress || `📦 ${item.product.name} - ${item.deliveryLat && item.deliveryLong ? 'Delivery set' : 'Default location'}`
        });
      });
  ...
  const handleProceedToDelivery = async () => {
    ...
    try {
      ...
      // Get the first actual delivery stop (skip warehouse if it's index 0)
      const firstDeliveryStop = optimizedRouteDetails.path.find(loc => 
        !loc.name.toLowerCase().includes('warehouse')
      ) || optimizedRouteDetails.path[1] || optimizedRouteDetails.path[0];
      ...
      const response = await fetch(`/api/packages/${packageId}`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          deliveryLat: firstDeliveryStop.latitude,
          deliveryLong: firstDeliveryStop.longitude,
          deliveryAddress: firstDeliveryStop.description || firstDeliveryStop.name,
          totalDistance: optimizedRouteDetails.totalDistance,
          routeAlgorithm: selectedAlgorithm,
          items: packageData.items.map((item, index) => ({
            id: item.id,
            deliveryLat: item.deliveryLat || deliveryLocations[index + 1]?.latitude,
            deliveryLong: item.deliveryLong || deliveryLocations[index + 1]?.longitude,
            deliveryAddress: item.deliveryAddress || deliveryLocations[index + 1]?.description,
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {location.description === 'Default location' && (
                            <span className="px-1.5 py-0.5 bg-yellow-100 text-yellow-700 text-[10px] font-medium rounded border border-yellow-300">
                              DEFAULT
                            </span>
                          )}
                        </div>
